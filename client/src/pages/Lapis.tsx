...
export function LapisReticulaCoordinatesDialog({ form, workspaceContext, disabled }: { form: LapisForm; workspaceContext?: string; disabled: boolean }) {
  const [open, setOpen] = useState(false);
  ...
  const openReticula = () => {
    ...
    setOpen(false);
    toast.success("Coordenadas enviadas ao Retícula para sua revisão.");
  };
...
export function LapisWorkspace({ onNavigateToCartographer = () => { window.location.href = "/discover"; } }: { onNavigateToCartographer?: () => void }) {
  ...
  const hasReticulaSeed = form.title.trim().length > 2 && [form.researchIdea, form.problemStatement, form.researchQuestion].some(value => value.trim().length > 2);
  const reticulaWorkspaceContext = [
