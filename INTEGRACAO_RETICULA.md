...
Os dois produtos também oferecem continuidade de descoberta por URL, sem uso de API privada, raspagem ou sincronização em segundo plano. O **Lapis** consolida uma proposta inicial de `theme`, `subject` e `discipline` a partir do título, ideia, problema, pergunta e, quando disponíveis, elementos metodológicos e análises do workspace. A abertura do Retícula permanece disponível tanto no Lapis quanto na etapa de literatura, dentro do **Cartographer**. Neste último, o AcademiaOS prioriza o caderno explicitamente recebido pela URL, depois o vínculo persistido com a revisão, a coincidência exata do título e uma correspondência forte e exclusiva entre termos significativos dos títulos. Quando dois cadernos se mostram igualmente semelhantes, a associação continua manual: o sistema não escolhe nem grava um vínculo silenciosamente. Para o recorte de ondas internas oceânicas com SAR e redes neurais, a proposta editável é `Ocean internal waves`, `Detection of ocean internal waves in Synthetic Aperture Radar (SAR) imagery using neural networks` e `Physical Oceanography`. Em qualquer outro caso, o tema, assunto e disciplina continuam sendo sugestões revisáveis na prévia. A ação principal abre a URL do Retícula sincronicamente a partir do clique do pesquisador e há um link de contingência com a mesma URL para cenários de bloqueio de pop-up. O link contém também `from=academiaos`. O Retícula apenas pré-preenche as coordenadas e exige que o pesquisador inicie a busca. No retorno, o Retícula abre `/search` do AcademiaOS em nova aba com a consulta e a proveniência `source=reticula`; o Cartographer mostra o contexto recebido e mantém o comando de busca sob controle explícito do pesquisador.
